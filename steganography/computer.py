...
def ensize():
    ...
    if len(lines) < 2 or first >= 100000:
        size = 0
    else:
...
def bianma():
    ...
    if n != 0:
        ...
        print(f"[{blue}{datetime.datetime.now().strftime('%H:%M:%S')}{end}] [{green}INFO{end}] {str(binary_numbers)}")
    ...
    if not n:
        ...
        if not key:
            def show_message():
                ...
                messagebox.showwarning("Attention", "Successfully hidden! Please take a screenshot and save it!")
...
def get_pos(board):
    ...
    get = (-1, -1)
    score = -float("inf")
    with open('candidate/choice.txt', 'w') as f:
        f.write('')
    scores = []
    for p in pos:
